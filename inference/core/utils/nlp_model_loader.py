...
class NLPModelLoader:
    ...
    def __init__(
        ...
        self.model_cache_directory = Path(model_cache_directory)
    ...
    def __download_model(self) -> None:
        ...
        tokenizer = self.tokenizer_loader.from_pretrained(f"{self.model_name}", cache_dir=self.model_cache_directory)
        model = self.model_loader.from_pretrained(f"{self.model_name}", cache_dir=self.model_cache_directory)

        logger.debug(f"[+] {self.save_path} does not exist!")
